...
function Dashboard({ session, onSessionChange }) {
  ...
  return <div className={`app-shell ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
    ...
    <main className="app">
      <header className="page-header"><div><small className="eyebrow">{isSettings ? 'Configuración' : 'Módulo'}</small><h1>{isSettings ? settingsPage === 'profiles' ? 'Búsquedas programadas' : settingsPage === 'appearance' ? 'Apariencia' : settingsPage === 'account' ? 'Credenciales de acceso' : 'Administrar cuenta' : isAgile ? 'Compras Ágiles' : 'Licitaciones'}</h1><p>{isProfiles ? 'Configura los criterios y alertas que quieres recibir.' : isSettings ? 'Administra la apariencia, tus credenciales y la sesión.' : isAgile ? 'Explora oportunidades de compra rápida.' : 'Encuentra procesos activos para ofertar.'}</p></div>{!isSettings && <span className="result-count"><strong>{total.toLocaleString('es-CL')}</strong> resultados</span>}</header>
      ...
      {isSettings && settingsPage === 'menu' && <section className="settings-layer panel"><div className="settings-user"><span>{session.user?.email?.slice(0, 1).toUpperCase()}</span><div><strong>{session.user?.email}</strong><small>Cuenta activa</small></div></div><h2>Administrar cuenta</h2><div className="settings-menu"><button onClick={() => openSettings('appearance')}><span>◐</span><div><strong>Apariencia</strong><small>Modo claro, oscuro o del sistema</small></div><b>›</b></button><button onClick={() => openSettings('account')}><span>♙</span><div><strong>Credenciales de acceso</strong><small>Correo electrónico y contraseña</small></div><b>›</b></button></div><button className="logout-button" onClick={logout}>Cerrar sesión</button></section>}

      {isSettings && settingsPage !== 'menu' && !isProfiles && <button className="settings-back" onClick={() => setSettingsPage('menu')}>← Volver a Administrar cuenta</button>}
